...
class WeightsObjective(Objective):
    ...
    def get_value(self, model, params, settings, stage_idx):
        cost = 0.0
        is_terminal = stage_idx == settings["N"]

        for idx, cost_function in enumerate(self._cost_functions):
            var_name = self._variables_per_function[idx]

            # acados terminal stage have not u variable
            if is_terminal and var_name in model.inputs:
                continue

            weights = []
            for cost_weight in self._weights_per_function[idx]:
                weights.append(params.get(cost_weight))

            variable = model.get(var_name)
