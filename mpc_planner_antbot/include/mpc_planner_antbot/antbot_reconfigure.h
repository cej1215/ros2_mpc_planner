...
#include <mpc_planner_antbot/antbotConfig.h>
...
public:
	AntbotReconfigure()
	{
		...
		ros::NodeHandle nh_reconfigure("mpc_planner_antbot");
		_reconfigure_server.reset(new dynamic_reconfigure::Server<mpc_planner_antbot::antbotConfig>(_reconfig_mutex, nh_reconfigure));
		_reconfigure_server->setCallback(boost::bind(&AntbotReconfigure::reconfigureCallback, this, _1, _2));
	}
	void reconfigureCallback(mpc_planner_antbot::antbotConfig &config, uint32_t level)
	{
		(void)level;
...
private:
	...
	boost::shared_ptr<dynamic_reconfigure::Server<mpc_planner_antbot::antbotConfig>> _reconfigure_server;
